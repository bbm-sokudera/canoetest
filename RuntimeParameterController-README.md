...
5. **GUIサイズの変更（文字が小さい場合）**

   **方法A: ボタンで変更**
   - GUIパネル上部の「GUI Scale」ボックス内のボタンをクリック
   - `0.8x`, `1.0x`, `1.5x`, `2.0x`, `2.5x` から選択

   **方法B: スライダーで微調整**
   - 「GUI Scale」のスライダーをドラッグ（0.5x〜3.0x）

   **方法C: キーボードショートカット**
   - **+ キー** (または テンキーの +) : GUIを拡大
   - **- キー** (または テンキーの -) : GUIを縮小

   > 💡 ヒント: デフォルトは1.5xに設定されています。文字が小さい場合は2.0xや2.5xをお試しください。
